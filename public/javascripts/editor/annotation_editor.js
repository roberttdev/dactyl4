dc.ui.AnnotationEditor = Backbone.View.extend({
  ...
  initialize: function(options) {
    ...
    _.bindAll(this, 'open', 'close', 'drawHighlight', 'saveHighlight', 'passCancelNotification',
      'deleteHighlight', 'selectHighlightPoint');
  ...
  open : function(highlight, groupId, showEdit, success, highlight_type) {
    ...
    this.hideActiveHighlights(function(){
      ...
      if( highlight.get('highlight_id') ){ return _me.showHighlight(highlight, highlight_type, showEdit); }

      if (highlight != null) {
        _me._active_highlight = highlight;
      }
  ...
  drawHighlight_dragEnd: function(e) {
    ...
    var image = _.map([loc.top, loc.right, loc.bottom, loc.left], function (l) {
    ...
    if (loc.width > 5 && loc.height > 5) {
      ...
      if (highlight_type == 'graph') {
          currentDocument.api.addHighlight({
              anno_type: 'graph',
              document_id: _highlight.get('document_id'),
              groups: _highlight.groups,
  ...
  saveHighlight : function(saveContent) {
    var unsaved = saveContent.unsaved;
    var params = this.contentToParams(saveContent);
  ...
  // Convert a content object into serializable params understood by us.
  contentToParams : function(anno) {
    delete anno.unsaved;
    var params = {
      id          : anno.server_id,
      content     : anno.text,
  ...
  selectHighlightPoint: function(highl_content) {
    if( this._open ){
      ...
      this._open = false;
      this.stopSelectionEvents();
      currentDocument.api.addContentToHighlight(highl_content.content.highlight_id, this._highlight_type);
    }else{
      if( highl_content.type = 'anno' ){ this.trigger('annotationSelected', this.contentToParams(highl_content.content)); }
      else if( highl_content.type = 'graph' ){ this.trigger('graphSelected', this.contentToParams(highl_content.content)); }
    }
  },
  ...
  //Pass highlight data to DV so it can update any missing IDs (either annotaton_id or graph_id along with location)
  syncDV: function(highlightInfo) {
      currentDocument.api.syncHighlightIDs(highlightInfo);
  },

  //Pass group association to DV so it can update if necessary
  syncGroupAssociation: function(annoId, groupId){
      currentDocument.api.syncGroupHighlight(annoId, groupId);
  },
